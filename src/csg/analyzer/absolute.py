...
def _iqr_stats(values: list[int]) -> dict:
    ...
    return {'median': statistics.median(values), 'iqr': q3 - q1, 'q3': q3, 'count': len(values)}
...
def analyze_absolute(filepath: Path, peer_stats: dict, corpus_stats: CorpusStats, cfg: dict) -> list[CheckResult]:
    ...
    if size >= fail_bytes:
        checks.append(CheckResult(check='absolute_size_floor_critical', score=2, detail=f'Size {size} B >= fail floor {fail_bytes} B', value=size))
    elif size >= warn_bytes:
        checks.append(CheckResult(check='absolute_size_floor_warn', score=1, detail=f'Size {size} B >= warn floor {warn_bytes} B', value=size))
    ...
    if peer and peer.get('count', 0) >= min_peer:
        ...
        if size > threshold and size > min_bytes:
            ...
            checks.append(CheckResult(check='absolute_peer_size_outlier', score=score, detail=f'Size {size} B > peer P95-ish {round(threshold)} B ({ext} in {source})', value=size))
    ...
    if corpus and corpus.get('sample_size', 0) >= min_corpus:
        ...
        if size > threshold and size > min_bytes:
            checks.append(CheckResult(check='absolute_corpus_size_outlier', score=1, detail=f'Size {size} B > corpus limit {round(threshold)} B ({ext})', value=size))
    return checks